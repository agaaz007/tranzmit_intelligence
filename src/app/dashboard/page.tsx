...
interface PrioritizedIssue {
  ...
  affected_archetypes?: Array<{ archetypeId: string; archetypeName: string }>;
}
...
${issue.affected_archetypes && issue.affected_archetypes.length > 0 ? `\n**Affected Archetypes:**\n${issue.affected_archetypes.map(a => `- ${a.archetypeName}`).join('\n')}` : ''}
---
...
export default function DashboardPage() {
  ...
  const loadDashboard = async (projId: string, forceRefresh = false) => {
    ...
    try {
      ...
      try {
        const archRes = await fetch(`/api/archetypes?projectId=${projId}`);
        const archData = await archRes.json();
        if (archData.archetypes) setArchetypes(archData.archetypes);
      } catch {}
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-1 bg-gray-100 dark:bg-[#141414] rounded-lg p-1">
              {(['unpaid', 'paid'] as const).map(tab => (
                ...
                >
                  {tab === 'unpaid' ? 'Trial Churn' : 'Paid Churn'}
                </button>
              ))}
            </div>
          </div>
