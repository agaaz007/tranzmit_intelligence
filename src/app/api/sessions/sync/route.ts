...
async function fetchSessionEvents(
  ...
  // Try to get snapshot sources (blob keys) with both URL patterns
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!listRes || listError) {
      return NextResponse.json({
        error: 'Failed to fetch sessions from PostHog',
        details: listError,
        hint: 'Check that your PostHog API key has read access to session recordings'
      }, { status: 502 });
    }
    ...
    if (posthogSessions.length === 0) {
      return NextResponse.json({
        ...
        failed: 0,
        errors: [],
        message: 'No session recordings found in PostHog. Make sure session replay is enabled.'
      });
    }
