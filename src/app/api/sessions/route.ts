...
export async function GET(req: NextRequest) {
  try {
    ...
    const sessionList: SessionListItem[] = sessions.map((s) => ({
      ...
      name: s.name,
      source: s.source as SessionListItem['source'],
      ...
      multimodalStatus: (s.multimodalStatus || 'pending') as 'pending' | 'analyzing' | 'completed' | 'failed',
      multimodalAnalysis: s.multimodalAnalysis ? JSON.parse(s.multimodalAnalysis) : undefined,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sessionItem: SessionListItem = {
      ...
      name: session.name,
      source: session.source as SessionListItem['source'],
      ...
      multimodalStatus: 'pending' as const,
      hasEvents: true,
